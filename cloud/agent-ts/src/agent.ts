...
export class VibeAgent {
  ...
  private handleResumeSession(sessionId: string, projectPath?: string): void {
    ...
    // Session/project switch is always allowed even during processing.
    // The current task keeps running in the background.
    // The next query will use the new session/cwd.
    this.lastProjectPath = historyPath;
